...
export async function uploadVisualReport(
  ...
  const reportId =
    options?.reportId ??
    `visual-${artifact.format}-${artifact.receiptRef.replace(/[^A-Za-z0-9._:-]/g, '').slice(0, 80)}`;
  ...
  if (!res.ok || !res.body?.location) {
    throw new Error(
      `visual report upload failed: HTTP ${res.status}${res.error ? ` — ${res.error}` : ''}`,
    );
  }
  ...
  return {
    ...
    receiptRef: res.body.receiptRef ?? artifact.receiptRef,
