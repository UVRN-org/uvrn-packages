/**
 * BP-08 — hygiene walls (checker package not in public export).
 */

import { existsSync, readdirSync, readFileSync } from 'fs';
import { join } from 'path';

describe('BP-08 hygiene walls', () => {
  it('identity sources stay free of d1/cloudflare/wrangler imports', () => {
  ...
  it('CheckerAttestation.identity remains a string when checker package present', () => {
    const checkerIndex = join(__dirname, '..', '..', '..', 'uvrn-checker', 'src', 'index.ts');
    if (!existsSync(checkerIndex)) return;
    const src = readFileSync(checkerIndex, 'utf8');
    expect(src).toMatch(/export interface CheckerAttestation \{[\s\S]*?identity: string;/);
  });
});
